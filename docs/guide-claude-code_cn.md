# Aegis MCP 接入指南 — Claude Code

本指南将引导你使用 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 作为 AI Agent 时，通过 Aegis 保护你的 MCP 工具服务器。
...
- 一个正在运行的 MCP 工具服务器
- 已安装 Claude Code（终端中可用 `claude` 命令）
...
3. **Agent 类型** — 选择 **Claude Code**（如已安装会自动检测）。向导会将 Aegis 代理地址注入 `~/.claude/mcp_servers.json`。
...
Claude Code 会自动检测 `~/.claude/mcp_servers.json` 的变更。启动（或重启）Claude Code，调用一个受保护的工具 — 它现在会通过 Aegis 路由。

```bash
# 检查 Aegis 健康状态
curl localhost:18070/health

# 查看审计日志 — 应该能看到你的工具调用记录
curl 'localhost:18070/api/v1/audit/logs?limit=5' | jq
```
...
2. **注入了**一条记录到 `~/.claude/mcp_servers.json`：
   ```json
   {
     "your-backend": {
...
3. **效果**：Claude Code 现在调用 Aegis 而非直接调用你的 MCP 服务器。Aegis 负责执行 ACL、限流、审批和审计日志，然后转发到后端。

### 架构

```
Claude Code  →  Aegis (:18070)  →  你的 MCP 服务器
```
